...
app.get('/.netlify/functions/api/contacts', async (req, res) => {
    try {
        const { name, lastname, saleNumber, dni, date, startDate, endDate, all } = req.query;
        let query = {};
...
app.post('/.netlify/functions/api/contacts', async (req, res) => {
    try {
        const newContact = await Contact.create(req.body);
        res.status(201).json(newContact);
    } catch (error) {
...
app.post('/.netlify/functions/api/contacts/import', async (req, res) => {
    try {
        const importedContactsData = req.body;
        const newContacts = [];
        const errors = [];
...
app.put('/.netlify/functions/api/contacts/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const updatedContact = await Contact.findByIdAndUpdate(id, req.body, { new: true, runValidators: true });
        if (!updatedContact) {
...
app.delete('/.netlify/functions/api/contacts/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const deletedContact = await Contact.findByIdAndDelete(id);
        if (!deletedContact) {
...
app.put('/.netlify/functions/api/contacts/:id/comments', async (req, res) => {
    try {
        const { id } = req.params;
        const { comments } = req.body;
        const updatedContact = await Contact.findByIdAndUpdate(id, { comments }, { new: true });
